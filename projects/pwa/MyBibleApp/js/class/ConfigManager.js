...
class ConfigManager {
	...
	getEnabledVersions() {
		return this.values.enabledVersions || [];
	}

	setEnabledVersions(abbreviations) {
		this.values.enabledVersions = abbreviations;
		this.saveValues();
	}

	isVersionEnabled(abbreviation) {
		return this.getEnabledVersions().includes(abbreviation);
	}

	enableVersion(abbreviation) {
		const current = this.getEnabledVersions();
		if (!current.includes(abbreviation)) {
			this.setEnabledVersions([...current, abbreviation]);
		}
	}

	disableVersion(abbreviation) {
		const current = this.getEnabledVersions();
		this.setEnabledVersions(current.filter(a => a !== abbreviation));
	}
}
