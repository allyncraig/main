...
class SearchManager extends Manager {
	constructor(versionManager, databaseManager, apiClient, resultsElementId, navigationManager, modalManager, configManager, chapterViewManager, contentRenderer) {
		super();
	...
	async _execSearch(searchTerm, version, page = 1) {
	...
	showLoading(isPagination = false) {
		if (isPagination) {
			...
			if (!loadingIndicator) {
				...
				loadingIndicator.innerHTML = HTML.SEARCH_SPINNER;
			...
			this.resultsElement.innerHTML = HTML.SEARCH_LOADING;
		}
	}
	...
	handleResultClick(element) {
		...
		this.handleSearchResultClick(bookAbbr, chapter, verse);
	}
	...
	renderPagination(pageInfo) {
		...
		if (totalPages <= 1) {
			paginationContainer.innerHTML = HTML.SEARCH_SINGLE_PAGE;
			return;
		}
		...
		for (let i = startPage; i <= endPage; i++) {
			...
			pageButtonsHTML += Template.interpolate(HTML.SEARCH_PAGE_NUMBER_BTN, {
				page: i,
				currentClass: isCurrentPage ? 'current' : '',
		...
		paginationContainer.innerHTML = Template.interpolate(HTML.SEARCH_PAGINATION, {
			...
			page, totalPages, firstRef, lastRef
		});
	}
	...
	showSearch() {
		...
		this.modalManager.show(MODAL.SEARCH, UI.SEARCHINPUT);
	}
	...
	async executeSearch(page = 1) {
		...
		const result = await this._execSearch(searchTerm, resolvedVersion, page);

		if (!result || result.error) {
			this.navigationManager.showNotification(result.error);
	...
	async navigateSearchPage(page) {
		await this.executeSearch(page);
	}

	// Global function for search result clicks
	async handleSearchResultClick(bookAbbr, chapter, verse) {
		...
		const verseNum = this.navigationManager.navigateToSearchResult(bookAbbr, chapter, verse);
		...
		if (verseNum) {
			this.contentRenderer.scrollToVerse(verseNum, true);
		}
		closeLoading();
	}
}