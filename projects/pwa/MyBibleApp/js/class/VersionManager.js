...
class VersionManager {
	constructor(versionConfig, databaseManager, apiClient, storageManager) {
		...
		this.currentBooks = [];
		this.resolvedsourceCache = new Map();
	}
	...
	async resolveVersionSource(version) {
		const cacheKey = version.abbreviation;
		if (this.resolvedsourceCache.has(cacheKey)) {
			return this.resolvedsourceCache.get(cacheKey);
		}
		...
		if (version.sources) {
			...
			if (version.sources.db) {
				...
				if (hasData) {
					...
					this.resolvedsourceCache.set(cacheKey, resolved);
					return resolved;
				}
			}
	...
	invalidateResolvedSourceCache(abbreviation) {
		this.resolvedsourceCache.delete(abbreviation);
	}
